...
func CreateBook(context *fiber.Ctx) error {
	...
	err = database.DBConn.Create(&book).Error
	if err != nil {
...
func DeleteBook(context *fiber.Ctx) error {
	...
	err := database.DBConn.Delete(bookModel, id)
...
func GetBooks(context *fiber.Ctx) error {
	...
	err := database.DBConn.Find(bookModels).Error
	if err != nil {
		context.Status(http.StatusBadRequest).JSON(
...
func GetBookByID(context *fiber.Ctx) error {
	...
	err := database.DBConn.Where("id = ?", id).First(bookModel).Error
	if err != nil {
		context.Status(http.StatusBadRequest).JSON(
